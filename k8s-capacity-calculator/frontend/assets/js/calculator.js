// calculator.js - Fixed and simplified
console.log('✅ calculator.js loaded');

import { calculateNamespaceCost } from "./calculatorCore.js";
import { renderResults, initializeModal, setResultsArray, setLoadingState } from "./uiRenderer.js";
...
function validateFormData(formData) {
  ...
  if (formData.cpu_req > formData.cpu_lim) {
    errors.push('CPU request cannot be greater than CPU limit');
  }
  if (formData.mem_req > formData.mem_lim) {
  ...
  try {
    ...
    document.getElementById("btnAdd").addEventListener("click", () => {
      ...
      if (!validation.isValid) {
        // Use the global showValidationErrors function
        if (typeof window.showValidationErrors === 'function') {
          window.showValidationErrors(validation.errors);
        } else {
          console.error('showValidationErrors function not found, using fallback');
      ...
      try {
        ...
        if (typeof window.showValidationErrors === 'function') {
          window.showValidationErrors(['Error calculating costs. Please check your inputs.']);
        } else {
    ...
    document.getElementById("btnFinalize").addEventListener("click", () => {
      if (!results.length) {
        if (typeof window.showValidationErrors === 'function') {
          window.showValidationErrors(['Please add at least one namespace before finalizing.']);
        } else {
          alert('Please add at least one namespace before finalizing.');
        }
        return;
      }
      ...
      try {
        if (window.finalizeModalManager) {
          ...
          if (typeof window.showValidationErrors === 'function') {
            window.showValidationErrors(['Finalize modal not available. Please refresh the page.']);
          } else {
            alert('Finalize modal not available. Please refresh the page.');
          }
        }
      } catch (error) {
        console.error('Error showing finalize modal:', error);
        if (typeof window.showValidationErrors === 'function') {
          window.showValidationErrors(['Error displaying summary. Please try again.']);
        } else {
          alert('Error displaying summary. Please try again.');
        }
      }
    });
    ...
    document.getElementById("btnExport")?.addEventListener("click", () => {
      if (!results.length) {
        if (typeof window.showValidationErrors === 'function') {
          window.showValidationErrors(['No data to export.']);
        } else {
      ...
      try {
        ...
        if (typeof window.showValidationErrors === 'function') {
          window.showValidationErrors(['Error exporting data.']);
        } else {
